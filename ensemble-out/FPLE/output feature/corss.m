...
%f:选择的不同特征维数，由数据集决定,例如f=(10,20,30,40,50)
[m,n]=size(sample);%获取数据集特征个数与样本个数
fs=size(f,2);%选取的特征的个数
...
for i=1:cross_num%进行交叉验证
    ...
    for j=1:fs
        [saccu_e1,kaccu_e1,saccu1,kaccu1,saccu_n1,kaccu_n1,weights_out_en1,weights_en_out1,weights_non1] =getaccu( train,test,e,e1,f(j),K);%进行分类准确率实验
        saccu_e2(j,:)=saccu_e1;
        kaccu_e2(j,:)=kaccu_e1;
        saccu2(j,:)=saccu1;
        kaccu2(j,:)=kaccu1;
