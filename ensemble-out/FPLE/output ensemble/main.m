...
sn=size(e,2);%获得隐私度个数
[L_saccu_out_en,L_kaccu_out_en,L_saccu_en_out,L_kaccu_en_out,L_saccu_non,L_kaccu_non,weights_out_en,weights_en_out,weights_non] = corss( sample,f,cm,e,e1,K);
%其中L_saccu_out_en是先加噪声后集成的svm分类器结果，L_kaccu_out_en是先加噪声后集成的knn分类器结果。weights_out_en：先加噪声后集成的权重
