...
for i=1:cross_num%进行交叉验证
    ...
    [saccu_e1,kaccu_e1,saccu1,kaccu1,saccu_n1,kaccu_n1,weights_out_en1,weights_en_out1,weights_non1] =getaccu( train,test,e,e1,f,K);%进行分类准确率实验
    saccuracy{i}=saccu1;
    kaccuracy{i}=kaccu1;
    saccuracy_n{i}=saccu_n1;
    kaccuracy_n{i}=kaccu_n1;
    saccuracy_e{i}=saccu_e1;
    kaccuracy_e{i}=kaccu_e1;
    weights_out_en3{i}=weights_out_en1;
...
result_saccu_e=0;%先加噪声后集成的最终准确率结果初始化
result_kaccu_e=0;%先加噪声后集成的最终准确率结果初始化
%%将10次交叉验证取平均
for i=1:cross_num
